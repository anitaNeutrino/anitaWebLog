...
$filehandle = fopen ($filename,"r") or die(mysql_error());
...
#this means a root file has been found, but nothing has been entered into the database so enter relevent info into the database
	    mysql_query("UPDATE runTable SET startTime='$array[2]' WHERE runNumber='$array[0]'") or die('error updating starttime');
	    mysql_query("UPDATE runTable SET endTime='$array[3]' WHERE runNumber='$array[0]'") or die('error updating endtime');
	    mysql_query("UPDATE runTable SET firstEvent='$array[4]' WHERE runNumber='$array[0]'") or die('error updating firstevent');
	    mysql_query("UPDATE runTable SET lastEvent='$array[5]' WHERE runNumber='$array[0]'") or die('error updating lastevent');
	  }
      }
    else
  ...
  //if($foundEntry<3){
    ...
    {#this means there is entry in the database yet, set all values
       mysql_query("INSERT INTO runTable (id,runNumber,location,startTime,endTime,firstEvent,lastEvent,shifterName,runDescription) VALUES('NULL','$theArray[0]','$theArray[1]','$theArray[2]','$theArray[3]','$theArray[4]','$theArray[5]','$theArray[6]','$theArray[7]')") or die(mysql_error());
    }  
  }
}
fclose($filehandle);
